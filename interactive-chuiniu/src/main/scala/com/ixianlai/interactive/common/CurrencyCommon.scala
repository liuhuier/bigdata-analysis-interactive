...
object CurrencyCommon {
    def selectOdsBrand(spark: SparkSession)(day: String,
        ...
        val temp3 = temp1.join(temp2, Seq("app_id", "user_id"))
                .select(temp1.col("app_id"), temp1.col("user_id"), temp2.col("target_day"))
        ...
        val temp5 = temp3.join(temp4, Seq("app_id", "user_id"))
                .select(temp3.col("app_id"), temp3.col("user_id"), temp4.col("guild_id"))
        ...
        // 亲友圈和好友桌总完整把数
        val temp6 = spark.read.table(edw_gameover_app).where($"target_day".cast("string") === s"$day").where($"dissolved" === "false")
                .withColumn("category", when($"guild_id" > 0, 1).otherwise(0))
                .select($"app_id", $"bs_play", $"category")
        ...
        var res3 = temp8.where($"category" === 1)
                .agg(sum($"bs_play").as("qinyouquan_bashu_count")).select($"qinyouquan_bashu_count")
                .withColumn("target_day", when($"qinyouquan_bashu_count" === 1, s"$day").otherwise(s"$day"))
        var res5 = temp8.where($"category" === 0)
                .agg(sum($"bs_play").as("haoyouzhuo_bashu_count")).select($"haoyouzhuo_bashu_count")
