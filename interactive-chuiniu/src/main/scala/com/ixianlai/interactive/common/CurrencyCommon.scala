...
object CurrencyCommon {
    def selectOdsBrand(spark: SparkSession)(day: String,
        ...
        // 登录用户中绑定吹牛的用户
        // target_day app_id user_id
        val temp3 = temp1.join(temp2, Seq("app_id", "user_id")).select(temp2.col("target_day"), temp2.col("app_id"), temp2.col("user_id"))

        // 结果一,登录用户绑定吹牛的用户总数
        val res1 = temp3.agg(count("*").as("all_count")).select($"target_day", $"all_count")
        ...
        // A中亲友圈和好友桌完整把数>=1的用户数
        val res2 = temp5.where($"new_guild_id" === 1).agg(count("*").as("qinyouquan_count")).select("target_day", "qinyouquan_count")
        val res4 = temp5.where($"new_guild_id" === 0).agg(count("*").as("haoyouzhuo_count")).select("target_day", "hzoyouzhuo_count")

        val res = res1.join(res2, "target_day").join(res4, "target_day")
                .select(res1.col("all_count"), res2.col("qinyouquan_count"), res4.col("hzoyouzhuo_count"))
        // res.rdd.map(_.mkString("|")).saveAsTextFile("file:")
        res.write.format("csv").save("file:///home/hadoop/liuhui")
    }
}
